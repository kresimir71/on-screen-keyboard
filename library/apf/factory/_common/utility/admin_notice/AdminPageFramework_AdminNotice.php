...
class OnScreenKeyboardAdminPageFramework_AdminNotice extends OnScreenKeyboardAdminPageFramework_FrameworkUtility {
    ...
    public function __construct($sNotice, array $aAttributes=array( 'class' => 'error' ), array $aCallbacks=array())
    {
        ...
        $this->aAttributes[ 'class' ] = $this->getClassAttribute($this->aAttributes[ 'class' ], 'on-screen-keyboard-settings-notice-message', 'on-screen-keyboard-settings-notice-container', 'notice', 'is-dismissible');
        $this->aCallbacks = $aCallbacks + $this->aCallbacks;
        new OnScreenKeyboardAdminPageFramework_AdminNotice___Script;
        if (! $sNotice) {
            return;
        }
