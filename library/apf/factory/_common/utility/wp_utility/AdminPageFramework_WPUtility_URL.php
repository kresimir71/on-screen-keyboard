...
class OnScreenKeyboardAdminPageFramework_WPUtility_URL extends OnScreenKeyboardAdminPageFramework_Utility {
    ...
    {
        $_sAdminURL = is_network_admin() ? network_admin_url(OnScreenKeyboardAdminPageFramework_WPUtility_Page::getPageNow()) : admin_url(OnScreenKeyboardAdminPageFramework_WPUtility_Page::getPageNow());
    ...
    public static function getSRCFromPath($sFilePath)
    {
        ...
        if (false !== strpos($sFilePath, $_sContentDirPath)) {
            $_sRelativePath = OnScreenKeyboardAdminPageFramework_Utility::getRelativePath(WP_CONTENT_DIR, $sFilePath);
            $_sRelativePath = preg_replace("/^\.[\/\\\]/", '', $_sRelativePath, 1);
            return content_url($_sRelativePath);
        }
        $_sRelativePath = OnScreenKeyboardAdminPageFramework_Utility::getRelativePath(ABSPATH, $sFilePath);
        $_sRelativePath = preg_replace("/^\.[\/\\\]/", '', $_sRelativePath, 1);
