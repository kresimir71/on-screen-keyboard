...
class OnScreenKeyboardAdminPageFramework_Form_View___Sectionsets extends OnScreenKeyboardAdminPageFramework_Form_View___Section_Base {
    ...
    public function get()
    {
        $_oFormatSectionsetsByTab = new OnScreenKeyboardAdminPageFramework_Form_View___Format_SectionsetsByTab($this->aStructure[ 'sectionsets' ], $this->aStructure[ 'fieldsets' ], $this->aArguments[ 'nested_depth' ]);
        $_aOutput = array();
        ...
        $_oDebugInfo = new OnScreenKeyboardAdminPageFramework_Form_View___DebugInfo($this->aArguments[ 'structure_type' ], $this->aCallbacks, $this->oMsg);
        $_sOutput = implode(PHP_EOL, $_aOutput);
        $_sElementID = "on-screen-keyboard-sectionsets-" . uniqid();
        return $this->___getSpinnerOutput($_sOutput) . "<div id='{$_sElementID}' class='on-screen-keyboard-sectionsets on-screen-keyboard-form-js-on'>" . $_sOutput . $_oDebugInfo->get() . "</div>" ;
    }
    private function ___getSpinnerOutput($_sOutput)
    {
        if (trim($_sOutput)) {
            return "<div class='on-screen-keyboard-form-loading' style='display: none;'>" . $this->oMsg->get('loading') . "</div>";
        }
        return '';
    }
    private function ___getFormOutput(array $aSectionsets, array $aFieldsets, $sSectionTabSlug, $aCallbacks)
    {
        ...
        return $_sSectionSet ? "<div " . $this->getAttributes(array( 'class' => 'on-screen-keyboard-sectionset', 'id' => "sectionset-{$sSectionTabSlug}_" . md5(serialize($aSectionsets)), )) . ">" . $_sSectionSet . "</div>" : '';
    }
    ...
    {
        $_oArgumentFormater = new OnScreenKeyboardAdminPageFramework_Form_Model___Format_CollapsibleSection($aSectionset[ 'collapsible' ], $aSectionset[ 'title' ], $aSectionset);
    ...
    private function ___getSectionsetTable($_aOutputs, $_sSectionsID, array $_aSection, array $aFieldsInSections)
    {
        ...
        $_oEachSectionArguments = new OnScreenKeyboardAdminPageFramework_Form_Model___Format_EachSection($_aSection, null, array(), $_sSectionsID);
    ...
    private function ___getSubSections($_aOutputs, $_sSectionsID, $_aSection, $_aSubSections)
    {
        if (! empty($_aSection[ 'repeatable' ])) {
            $_aOutputs[ 'section_contents' ][] = OnScreenKeyboardAdminPageFramework_Form_View___SectionRepeatableButtons::get($_sSectionsID, $_aOutputs[ 'count_subsections' ], $_aSection[ 'repeatable' ], $this->oMsg);
        ...
        foreach ($_aSubSections as $_iIndex => $_aFields) {
            $_oEachSectionArguments = new OnScreenKeyboardAdminPageFramework_Form_Model___Format_EachSection($_aSection, $_iIndex, $_aSubSections, $_sSectionsID);
    ...
    private function ___getSectionTableWithTabList(array $_aOutputs, array $aSectionset, $aFieldsetsPerSection)
    {
        ...
        $_oSectionTable = new OnScreenKeyboardAdminPageFramework_Form_View___Section($this->aArguments, $aSectionset, $this->aStructure, $aFieldsetsPerSection, $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->aCallbacks, $this->oMsg);
    ...
    private function ___getFormattedSectionsTablesOutput(array $aOutputs, $aSectionset, $sSectionsID, array $aCollapsible, $sSectionTabSlug)
    {
        ...
        $_oCollapsibleSectionTitle = new OnScreenKeyboardAdminPageFramework_Form_View___CollapsibleSectionTitle(array( 'title' => $this->getElement($aCollapsible, 'title', ''), 'tag' => 'h3', 'section_index' => null, 'collapsible' => $aCollapsible, 'container_type' => 'sections', 'sectionset' => $aSectionset, ), array(), $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->oMsg, $this->aCallbacks);
        $_oSectionsTablesContainerAttributes = new OnScreenKeyboardAdminPageFramework_Form_View___Attribute_SectionsTablesContainer($aSectionset, $sSectionsID, $sSectionTabSlug, $aCollapsible, $aOutputs[ 'count_subsections' ]);
    ...
    {
        return $sSectionTabSlug ? "<ul class='on-screen-keyboard-section-tabs nav-tab-wrapper'>" . implode(PHP_EOL, $aSectionTabList) . "</ul>" : '';
    }
    private function ___getTabList(array $aSection, array $aFields, $hfFieldCallback)
    {
        ...
        $_aTabAttributes = $aSection[ 'attributes' ][ 'tab' ] + array( 'class' => 'on-screen-keyboard-section-tab nav-tab', 'id' => "section_tab-{$_sSectionTagID}", 'style' => null );
        ...
        $_oSectionTitle = new OnScreenKeyboardAdminPageFramework_Form_View___SectionTitle(array( 'title' => $aSection[ 'title' ], 'tag' => 'h4', 'section_index' => $iSectionIndex, 'sectionset' => $aSection, ), $aFields, $this->aSavedData, $this->aFieldErrors, $this->aStructure[ 'field_type_definitions' ], $this->oMsg, $this->aCallbacks);
