...
(function($){
  ...
  /** global: OnScreenKeyboardAdminPageFrameworkFieldTypeTextArea */
  var apfTextArea = OnScreenKeyboardAdminPageFrameworkFieldTypeTextArea;
  ...
  $( document ).ready( function(){
    ...
    var updateFoundEditors = function ( oElements ) {
      ...
      oElements.each( function ( iIndex ) {
        ...
        var oSettings = $().getOnScreenKeyboardAdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );
    ...
    $().registerOnScreenKeyboardAdminPageFrameworkCallbacks( {
        ...
        repeated_field: function ( oCloned, aModel ) {
          ...
          var _oSettings = $().getOnScreenKeyboardAdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );

          // Elements
          var _oField = oCloned.closest( '.on-screen-keyboard-field' );
        ...
        stopped_sorting_fields: function ( oSortedFields, sFieldType, sFieldsTagID, iCallType ) {
          ...
          // Update the editor.
          setTimeout( function () {
            var _oFields = oSortedFields.children( '.on-screen-keyboard-field' );
        ...
        stopped_sorting_sections: function ( oSections ) {
          ...
          setTimeout( function () {
            var _oFields = $( oSections ).find( '.on-screen-keyboard-field' );
            updateFoundEditors( _oFields );
          }, 100 );
        ...
        saved_widget: function ( oWidget ) {
          ...
          $( oWidget ).find( '.on-screen-keyboard-field' ).each( function ( iIndex ) {
            ...
            // Store the settings.
            $().storeOnScreenKeyboardAdminPageFrameworkInputOptions(
              oWrap.attr( 'data-id' ),
              {
    ...
    // Initialize
    $( '.on-screen-keyboard-textarea-data-input' ).each( function(){
