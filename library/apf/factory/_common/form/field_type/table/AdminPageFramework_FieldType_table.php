...
class OnScreenKeyboardAdminPageFramework_FieldType_table extends OnScreenKeyboardAdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'on-screen-keyboard-field-type-table', 'src' => dirname(__FILE__) . '/js/table.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'jquery-ui-accordion', 'on-screen-keyboard-script-form-main' ), 'translation_var' => 'OnScreenKeyboardAdminPageFrameworkFieldTypeTable', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'label' => array(), ), ), );
    }
    ...
    private function ___getLabel($aField)
    {
        ...
        return "<div class='on-screen-keyboard-input-label-container'>" . "<label for='" . esc_attr($aField[ 'input_id' ]) . "'>" . "<span " . $this->getLabelContainerAttributes($aField, 'on-screen-keyboard-input-label-string') . ">" . $aField[ 'label' ] . "</span>" . "</label>" . "</div>";
    }
    private function ___getTable($aField)
    {
        ...
        $_sCaption = $aField[ 'caption' ] ? $aField[ 'caption' ] : __('Set the caption with the <code>caption</code> argument.', 'on-screen-keyboard');
