...
class OnScreenKeyboardAdminPageFramework_FieldType_image extends OnScreenKeyboardAdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'on-screen-keyboard-field-type-image', 'src' => dirname(__FILE__) . '/js/image.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'on-screen-keyboard-script-form-main' ), 'translation_var' => 'OnScreenKeyboardAdminPageFrameworkImageFieldType', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'referer' => 'admin_page_framework', 'hasMediaUploader' => function_exists('wp_enqueue_media'), 'label' => array( 'uploadImage' => $this->oMsg->get('upload_image'), 'useThisImage' => $this->oMsg->get('use_this_image'), 'insertFromURL' => $this->oMsg->get('insert_from_url'), ), ), ), );
    }
...
jQuery(document).ready(function(){setOnScreenKeyboardAdminPageFrameworkImageUploader('{$sInputID}','true'==='{$_sRepeatable}','true'==='{$_bExternalSource}')})
JAVASCRIPTS;
        return "<script type='text/javascript' class='on-screen-keyboard-image-uploader-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='on-screen-keyboard-{$sType}-remove-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
