...
class OnScreenKeyboardAdminPageFramework_FieldType_system extends OnScreenKeyboardAdminPageFramework_FieldType {
    ...
    protected function getField($aField)
    {
        ...
        return $aField[ 'before_label' ] . "<div class='on-screen-keyboard-input-label-container'>" . "<label for='{$aField[ 'input_id' ]}'>" . $aField[ 'before_input' ] . ($aField[ 'label' ] && ! $aField[ 'repeatable' ] ? "<span " . $this->getLabelContainerAttributes($aField, 'on-screen-keyboard-input-label-string') . ">" . $aField[ 'label' ] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->___getSystemInformation($aField[ 'value' ], $aField[ 'data' ], $aField[ 'print_type' ])) . "</textarea>" . $aField[ 'after_input' ] . "</label>" . "</div>" . $aField[ 'after_label' ];
    }
    ...
    private function ___getFormattedSystemInformation($asCustomData)
    {
        ...
        $_aData = $_aData + array( 'WordPress' => $this->___getSiteInfoWithCache(! isset($_aData[ 'WordPress' ])), 'PHP' => $this->_getPHPInfo(! isset($_aData[ 'PHP' ])), 'PHP Error Log' => $this->___getErrorLogByType('php', ! isset($_aData[ 'PHP Error Log' ])), 'MySQL' => isset($_aData[ 'MySQL' ]) ? null : $this->getMySQLInfo(), 'MySQL Error Log' => $this->___getErrorLogByType('mysql', ! isset($_aData[ 'MySQL Error Log' ])), 'Server' => $this->___getWebServerInfo(! isset($_aData[ 'Server' ])), 'Browser' => $this->___getClientInfo(! isset($_aData[ 'Browser' ])), 'Admin Page Framework' => isset($_aData[ 'Admin Page Framework' ]) ? null : OnScreenKeyboardAdminPageFramework_Registry::getInfo(), );
    ...
    private function ___getClientInfo($bGenerateInfo=true)
    {
        ...
        return empty($_aBrowser) ? __('No browser information found.', 'on-screen-keyboard') : $_aBrowser;
    }
    ...
    {
        global $wpdb;
        return array( __('Version', 'on-screen-keyboard') => $GLOBALS[ 'wp_version' ], __('Language', 'on-screen-keyboard') => $this->getSiteLanguage(), __('Memory Limit', 'on-screen-keyboard') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'on-screen-keyboard') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'on-screen-keyboard') => get_option('permalink_structure'), __('Active Theme', 'on-screen-keyboard') => $this->___getActiveThemeName(), __('Registered Post Statuses', 'on-screen-keyboard') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'on-screen-keyboard') => $wpdb->prefix, __('Table Prefix Length', 'on-screen-keyboard') => strlen($wpdb->prefix), __('Table Prefix Status', 'on-screen-keyboard') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->___getWPRemotePostStatus(), 'wp_remote_get()' => $this->___getWPRemoteGetStatus(), __('Multibite String Extension', 'on-screen-keyboard') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'on-screen-keyboard') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'on-screen-keyboard') => PHP_EOL . $this->___getActivePlugins(), __('Network Active Plugins', 'on-screen-keyboard') => PHP_EOL . $this->___getNetworkActivePlugins(), __('Constants', 'on-screen-keyboard') => $this->___getDefinedConstants('user'), );
    }
    ...
    private function _getPHPInfo($bGenerateInfo=true)
    {
        ...
        $_oErrorReporting = new OnScreenKeyboardAdminPageFramework_ErrorReporting;
        self::$_aPHPInfo = array( __('Version', 'on-screen-keyboard') => phpversion(), __('Safe Mode', 'on-screen-keyboard') => $this->getAOrB(@ini_get('safe_mode'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Memory Limit', 'on-screen-keyboard') => @ini_get('memory_limit'), __('Upload Max Size', 'on-screen-keyboard') => @ini_get('upload_max_filesize'), __('Post Max Size', 'on-screen-keyboard') => @ini_get('post_max_size'), __('Upload Max File Size', 'on-screen-keyboard') => @ini_get('upload_max_filesize'), __('Max Execution Time', 'on-screen-keyboard') => @ini_get('max_execution_time'), __('Max Input Vars', 'on-screen-keyboard') => @ini_get('max_input_vars'), __('Argument Separator', 'on-screen-keyboard') => @ini_get('arg_separator.output'), __('Allow URL File Open', 'on-screen-keyboard') => $this->getAOrB(@ini_get('allow_url_fopen'), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Display Errors', 'on-screen-keyboard') => $this->getAOrB(@ini_get('display_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Log Errors', 'on-screen-keyboard') => $this->getAOrB(@ini_get('log_errors'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Error log location', 'on-screen-keyboard') => @ini_get('error_log'), __('Error Reporting Level', 'on-screen-keyboard') => $_oErrorReporting->getErrorLevel(), __('FSOCKOPEN', 'on-screen-keyboard') => $this->getAOrB(function_exists('fsockopen'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('cURL', 'on-screen-keyboard') => $this->getAOrB(function_exists('curl_init'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SOAP', 'on-screen-keyboard') => $this->getAOrB(class_exists('SoapClient'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), __('SUHOSIN', 'on-screen-keyboard') => $this->getAOrB(extension_loaded('suhosin'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), 'ini_set()' => $this->getAOrB(function_exists('ini_set'), $this->oMsg->get('supported'), $this->oMsg->get('not_supported')), ) + $this->getPHPInfo() + array( __('Constants', 'on-screen-keyboard') => $this->___getDefinedConstants(null, 'user') ) ;
        return self::$_aPHPInfo;
    }
    private function ___getWebServerInfo($bGenerateInfo=true)
    {
        return $bGenerateInfo ? array( __('Web Server', 'on-screen-keyboard') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'on-screen-keyboard') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'on-screen-keyboard') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'on-screen-keyboard') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'on-screen-keyboard') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'on-screen-keyboard') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'on-screen-keyboard') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), ) + $_SERVER : '';
    }
}
