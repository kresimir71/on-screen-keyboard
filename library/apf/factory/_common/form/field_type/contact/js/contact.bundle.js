...
(function($){
  ...
  var apfContact = OnScreenKeyboardAdminPageFrameworkContactFieldType;
  ...
  function sendEmail( event ) {
    ...
    // Check required
    var _requiredInputs = $( this ).closest( '.on-screen-keyboard-sections' )
    ...
    var _oSpinner = $('<img src="' + apfMain.spinnerURL + '" alt="' + apfMain.messages.loading + '" />' )
      .addClass( 'on-screen-keyboard-ajax-spinner' );
    $( this ).closest( '.on-screen-keyboard-field' ).find( '.result-placeholder' ).prepend( _oSpinner );
    var _resultIcon = $( this ).closest( '.on-screen-keyboard-field' ).find( '.result-placeholder .dashicons' );
    ...
    $.ajax( {
        ...
        success: function ( response ) {
          ...
          if ( response.result ) {
            _resultIcon.addClass( 'dashicons-yes-alt success on-screen-keyboard-form-tooltip' );
            _resultIcon.prepend( "<span class='on-screen-keyboard-form-tooltip-content'>" + response.message + "</span>" );
          } else {
            _resultIcon.addClass( 'dashicons-warning error on-screen-keyboard-form-tooltip' );
            _resultIcon.prepend( "<span class='on-screen-keyboard-form-tooltip-content'>" + response.message + "</span>" );
          }
        },
        error: function( response ) {
          ...
          _resultIcon.addClass( 'dashicons-warning error on-screen-keyboard-form-tooltip' );
          _resultIcon.prepend( "<span class='on-screen-keyboard-form-tooltip-content'>" + response.status + ' ' + response.statusText + "</span>" );
        },
        complete: function() {
          _oSpinner.remove();
          debugLog( 'Request done.' );
          _resultIcon[ 'on-screen-keyboard-form-tooltip' ]({
              autoClose: false,
              shown: true,
              oneOff: true,
              noArrow: true,
