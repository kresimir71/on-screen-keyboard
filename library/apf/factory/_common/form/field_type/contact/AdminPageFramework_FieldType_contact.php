...
class OnScreenKeyboardAdminPageFramework_FieldType_contact extends OnScreenKeyboardAdminPageFramework_FieldType_submit {
    public $aFieldTypeSlugs = array( 'contact', );
    private $___sAction = 'on-screen-keyboard_contact_field_type_email';
    ...
    {
        return array( array( 'handle_id' => 'on-screen-keyboard-field-type-contact', 'src' => dirname(__FILE__) . '/js/contact.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'on-screen-keyboard-script-form-main' ), 'translation_var' => 'OnScreenKeyboardAdminPageFrameworkContactFieldType', 'translation' => array( 'nonce' => wp_create_nonce(get_class($this)), 'action' => $this->___sAction, 'messages' => array( 'requiredField' => $this->oMsg->get('please_fill_out_this_field'), ), ), ), );
    }
    ...
    private function ___doAjaxResponse()
    {
        ...
        $_oEmail = new OnScreenKeyboardAdminPageFramework_FormEmail($_aEmailOptions, $_aInputs, $_sSubmitSectionID);
