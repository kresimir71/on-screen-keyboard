...
(function($){
  ...
  $( document ).ready( function(){
    ...
    $( '.on-screen-keyboard-field-submit .submit-confirm-container input[type=checkbox]' ).each( function( index, value ){
      $( this ).closest( '.on-screen-keyboard-field-submit' ).find( 'input[type=submit]' ).on( 'click', function( event ){
        var _fieldSubmit = $( this ).closest( '.on-screen-keyboard-field-submit' );
        _fieldSubmit.find( '.submit-confirmation-warning' ).remove(); // previous error message
        var _confirmCheckbox = $( this ).closest( '.on-screen-keyboard-field-submit' ).find( '.submit-confirm-container input[type=checkbox]' );
