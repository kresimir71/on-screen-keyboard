...
( function( $ ) {
    ...
    $( document ).ready( function() {
        $( this ).initializeOnScreenKeyboardAdminPageFrameworkCollapsibleSections();
    });
    ...
    $( document ).on( 'on-screen-keyboard_saved_widget', function( event, oWidget ){
        $( oWidget ).initializeOnScreenKeyboardAdminPageFrameworkCollapsibleSections();
    });

    $.fn.initializeOnScreenKeyboardAdminPageFrameworkCollapsibleSections = function() {

        // Expand collapsible sections that are set not to collapse by default
        $( this ).find( '.on-screen-keyboard-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.on-screen-keyboard-collapsible-sections-content' )
            .slideDown( 'fast' );
        $( this ).find( '.on-screen-keyboard-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.on-screen-keyboard-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );

        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        $( this ).find( '.on-screen-keyboard-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.on-screen-keyboard-section-table' )
            .find( 'tbody' )
            .hide();

        // Bind the click event to the title element.
        $( this ).find( '.on-screen-keyboard-collapsible-sections-title, .on-screen-keyboard-collapsible-section-title' )
            .enableOnScreenKeyboardAdminPageFrameworkCollapsibleButton();

        // Insert the toggle all button.
        $( this ).find( '.on-screen-keyboard-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){$

            var _oThis        = $( this ); // to access from inside the below each() method.
            var _bForSections = $( this ).hasClass( 'on-screen-keyboard-collapsible-sections-title' );   // or for the 'section' container.
            ...
            $.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';
                _oButton.find( '.on-screen-keyboard-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.on-screen-keyboard-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.on-screen-keyboard-section' )[ _sTopOrBottom ]( _oButton );
                }
                ...
                _oButton.on( 'click', function(){
                    ...
                    var _oButtons = _bForSections
                        ? $( this ).closest( '.on-screen-keyboard-sectionset' ).siblings().addBack().find( '> .on-screen-keyboard-collapsible-toggle-all-button-container' )
                        : $( this ).siblings( '.on-screen-keyboard-collapsible-toggle-all-button-container' ).addBack();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .on-screen-keyboard-collapsible-title' ).each( function() {
                            $( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.on-screen-keyboard-sections' ).children( '.on-screen-keyboard-section' ).children( '.on-screen-keyboard-section-table' ).children( 'caption' ).children( '.on-screen-keyboard-collapsible-title' ).each( function() {
                            $( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
        ...
        function _getButtonContainer() {
            var _sDashIconSort = $.fn.compareVersionNumbers( OnScreenKeyboardAdminPageFrameworkScriptFormMain.wpVersion, '3.8' ) >= 0
                ? 'dashicons dashicons-sort'
                : '';
            var _sButtonText   = _sDashIconSort ? '' : OnScreenKeyboardAdminPageFrameworkScriptFormMain.messages.toggleAll;
            var _oButtonInner = $( '<span class="on-screen-keyboard-collapsible-toggle-all-button button"></span>' );
            _oButtonInner.addClass( _sDashIconSort );
            _oButtonInner.attr( 'title', OnScreenKeyboardAdminPageFrameworkScriptFormMain.messages.toggleAllCollapsibleSections );
            _oButtonInner.text( _sButtonText );
            var _oButton = $( '<div class="on-screen-keyboard-collapsible-toggle-all-button-container"></div>' );
     ...
     */
    $.fn.enableOnScreenKeyboardAdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( $( nodeTarget ).hasClass( 'on-screen-keyboard-collapsible-button' ) ) {
                return false;
            }
        ...
        $( this ).on( 'click', function( event, sContext ){
            ...
            // Expand or collapse this panel
            var _oThis = $( this );
            var _sContainerType = $( this ).hasClass( 'on-screen-keyboard-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? $( this ).next( '.on-screen-keyboard-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .on-screen-keyboard-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    $( this ).prev( '.on-screen-keyboard-collapsible-title' ).addClass( 'collapsed' );
                });
            }
