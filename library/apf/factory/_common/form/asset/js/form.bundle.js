...
(function ( $ ) {
     ...
     */
    $.fn.selectAllOnScreenKeyboardAdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
     ...
     */
    $.fn.deselectAllOnScreenKeyboardAdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
            .find( 'input[type=checkbox]' )
            .prop( 'checked', false )
            .trigger( 'change' );   // 3.8.8+
...
( function( $ ) {
    ...
    var _removeOnScreenKeyboardAdminPageFrameworkLoadingOutputs = function() {

        jQuery( '.on-screen-keyboard-form-loading' ).remove();
        jQuery( '.on-screen-keyboard-form-js-on' )
            .hide()
            .css( 'visibility', 'visible' )
            .fadeIn( 200 )
            .removeClass( '.on-screen-keyboard-form-js-on' );
    ...
    window.onerror = function(){
        ...
        _removeOnScreenKeyboardAdminPageFrameworkLoadingOutputs();
     ...
     * @since    3.7.0
     */
    $( document ).on( 'on-screen-keyboard_saved_widget', function( event, oWidget ){
        jQuery( '.on-screen-keyboard-form-loading' ).remove();
    });
...
(function ( $ ) {
    ...
    $.fn.aOnScreenKeyboardAdminPageFrameworkInputOptions = {};

    $.fn.storeOnScreenKeyboardAdminPageFrameworkInputOptions = function( sID, vOptions ) {
        ...
        $.fn.aOnScreenKeyboardAdminPageFrameworkInputOptions[ sID ] = vOptions;
    };
    $.fn.getOnScreenKeyboardAdminPageFrameworkInputOptions = function( sID ) {
        sID = sID.replace( /__\d+_/, '___' ); // remove the section index
        return ( 'undefined' === typeof $.fn.aOnScreenKeyboardAdminPageFrameworkInputOptions[ sID ] )
            ? null
            : $.fn.aOnScreenKeyboardAdminPageFrameworkInputOptions[ sID ];
    }
...
(function ( $ ) {
    ...
    // Callback containers.
    $.fn.aOnScreenKeyboardAdminPageFrameworkAddRepeatableFieldCallbacks        = [];
    $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aOnScreenKeyboardAdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aOnScreenKeyboardAdminPageFrameworkSortedFieldsCallbacks              = [];
    $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aOnScreenKeyboardAdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+

    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */
    $( document ).on( 'on-screen-keyboard_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aOnScreenKeyboardAdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerOnScreenKeyboardAdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     *
     * Unlike the `on-screen-keyboard_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).on( 'on-screen-keyboard_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).on( 'on-screen-keyboard_stopped_sorting_sections', function( oEvent ){

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).on( 'on-screen-keyboard_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aOnScreenKeyboardAdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aOnScreenKeyboardAdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     */
    $( document ).on( 'on-screen-keyboard_saved_widget', function( event, oWidget ){
        $.each( $.fn.aOnScreenKeyboardAdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 'function' !== typeof _hfCallback ) {
                return true;    // continue
     ...
     */
    $.fn.registerOnScreenKeyboardAdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aOnScreenKeyboardAdminPageFrameworkAddRepeatableFieldCallbacks.push(
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );
        $.fn.aOnScreenKeyboardAdminPageFrameworkRemoveRepeatableFieldCallbacks.push(
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aOnScreenKeyboardAdminPageFrameworkSortedFieldsCallbacks.push(
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingFieldsCallbacks.push(
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aOnScreenKeyboardAdminPageFrameworkAddedWidgetCallbacks.push(
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aOnScreenKeyboardAdminPageFrameworkStoppedSortingSectionsCallbacks.push(
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );

    };
    /**
     * An alias of the `registerOnScreenKeyboardAdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registerOnScreenKeyboardAdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.on-screen-keyboard-fields.repeatable' ).each( function() {
            var _buttonModel = $( this ).siblings( '.repeatable-field-buttons-model' );
            if ( ! _buttonModel.length ) {
                return true;
            }
            var _aSettings           = _buttonModel.data();
            var _buttonContainer     = _buttonModel.children( '.on-screen-keyboard-repeatable-field-buttons' ).first();
            ...
            var _childFields = $( this ).find( '> .on-screen-keyboard-field.without-child-fields' );
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.on-screen-keyboard-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .on-screen-keyboard-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.on-screen-keyboard-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.on-screen-keyboard-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updateOnScreenKeyboardAdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var OnScreenKeyboardAdminPageFrameworkScriptFormMain
     */
    var translation = OnScreenKeyboardAdminPageFrameworkScriptFormMain;
     ...
     */
    $.fn.updateOnScreenKeyboardAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.on-screen-keyboard-repeatable-field-buttons' )
            .filter( function() {
                return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            });
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addOnScreenKeyboardAdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removeOnScreenKeyboardAdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.on-screen-keyboard-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.on-screen-keyboard-field' )
            .filter( function() {
                return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            })
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {
                $( '#' + _sFieldID ).addOnScreenKeyboardAdminPageFrameworkRepeatableField( _sFieldID );
            }
        }
     ...
     */
    $.fn.addOnScreenKeyboardAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {

        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.on-screen-keyboard-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.on-screen-keyboard-fields' );
        ...
        if ( ! _aOptions ) {
            var _nodeButtonContainer = nodeFieldContainer.find( '.on-screen-keyboard-repeatable-field-buttons' )
                .filter( function() {
                    return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.on-screen-keyboard-field' )
                                    .filter( function() {
                                        return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.on-screen-keyboard-repeatable-field-buttons' )
                                    .filter( function() {
                                        return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
         ...
         */
        nodeNewField.updateOnScreenKeyboardAdminPageFrameworkRepeatableFields();
        ...
        nodeNewField.trigger(
            'on-screen-keyboard_added_repeatable_field',
            [
        ...
        $( nodeNewField ).find( '.on-screen-keyboard-field' ).addBack().trigger(
            'on-screen-keyboard_repeated_field',
            [
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.on-screen-keyboard-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {
                return $( this ).closest( '.on-screen-keyboard-fields' ).attr( 'id' ) === _sFieldsContainerID;
            })
        ...
        nodeFieldsContainer.children( '.on-screen-keyboard-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.on-screen-keyboard-field,.on-screen-keyboard-fields,.on-screen-keyboard-fieldset' ).incrementAttributes(
                [ 'id', 'data-field_tag_id_model', 'data-field_id' ],
                _iFieldCount,
                _sFieldTagIDModel
            );
            oElement.find( '.on-screen-keyboard-fields' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeOnScreenKeyboardAdminPageFrameworkRepeatableField = function() {

        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.on-screen-keyboard-field' );
        var nodeFieldsContainer = $( this ).closest( '.on-screen-keyboard-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.on-screen-keyboard-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.on-screen-keyboard-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.on-screen-keyboard-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.on-screen-keyboard-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.on-screen-keyboard-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updateOnScreenKeyboardAdminPageFrameworkRepeatableSections( _repeatOptions );
     ...
     */
    $.fn.updateOnScreenKeyboardAdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.on-screen-keyboard-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {
        ...
        var _aOptions = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).on( 'click', function() {
            ...
            $( this ).addOnScreenKeyboardAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // The Remove button behavior
        $( _oThis ).find( '.repeatable-section-remove-button' ).on( 'click', function() {
            $( this ).removeOnScreenKeyboardAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // If the number of sections is less than the set minimum value, add sections.
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.on-screen-keyboard-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.on-screen-keyboard-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {
                $( '#' + _sSectionID ).addOnScreenKeyboardAdminPageFrameworkRepeatableSection( _sSectionID );
            }
        }
     ...
     */
    $.fn.addOnScreenKeyboardAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {

        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.on-screen-keyboard-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.on-screen-keyboard-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.on-screen-keyboard-section-tabs-contents' )
            .children( '.on-screen-keyboard-section-tabs' )
            .first();

        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );

        var _iFadein                = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        var _bPreserveValues        = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'preserve_values' ];

        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.on-screen-keyboard-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.on-screen-keyboard-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.on-screen-keyboard-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.on-screen-keyboard-section-title' ).not( '.on-screen-keyboard-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableOnScreenKeyboardAdminPageFrameworkCollapsibleButton ){
            nodeNewSection.find( '.on-screen-keyboard-collapsible-sections-title, .on-screen-keyboard-collapsible-section-title' ).enableOnScreenKeyboardAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.on-screen-keyboard-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updateOnScreenKeyboardAdminPageFrameworkRepeatableFields();
            ...
            $( this ).trigger(
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.on-screen-keyboard-sections' )    // model container
                ]
            );
        ...
        nodeNewSection.updateOnScreenKeyboardAdminPageFrameworkRepeatableSections();

        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.on-screen-keyboard-fields.sortable' ).each( function() {
            $( this ).enableOnScreenKeyboardAdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.on-screen-keyboard-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.on-screen-keyboard-fieldrow, .on-screen-keyboard-fieldset, .on-screen-keyboard-fields, .on-screen-keyboard-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.on-screen-keyboard-fields' ).incrementAttribute(
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.on-screen-keyboard-fieldset' ).incrementAttribute(
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeOnScreenKeyboardAdminPageFrameworkRepeatableSection = function() {

        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.on-screen-keyboard-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.on-screen-keyboard-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.on-screen-keyboard-section-tabs-contents' )
            .children( '.on-screen-keyboard-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.on-screen-keyboard-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aOnScreenKeyboardAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.on-screen-keyboard-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.on-screen-keyboard-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() {
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.on-screen-keyboard-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.on-screen-keyboard-section-title' ).first().show();
                }
            }
        ...
        _oNextAllSections.each( function( _iIterationIndex ) {
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.on-screen-keyboard-field' ).each( function( iFieldIndex ) {
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.on-screen-keyboard-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.on-screen-keyboard-fields.sortable' ).each( function() {
            $( this ).enableOnScreenKeyboardAdminPageFrameworkSortableFields();
        } );
    });

    $.fn.enableOnScreenKeyboardAdminPageFrameworkSortableFields = function( sFieldsContainerID ) {
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.on-screen-keyboard-sections.sortable-section' ).each( function(){
            $( this ).enableOnScreenKeyboardAdminPageFrameworkSortableSections();
        } );
    });

    $.fn.enableOnScreenKeyboardAdminPageFrameworkSortableSections = function( sSectionsContainerID ) {
        ...
        var _bIsTabbed      = _oTarget.hasClass( 'on-screen-keyboard-section-tabs-contents' );
        var _bCollapsible   = 0 < _oTarget.children( '.on-screen-keyboard-section.is_subsection_collapsible' ).length;

        _oTarget        = _bIsTabbed
            ? _oTarget.find( 'ul.on-screen-keyboard-section-tabs' )
            : _oTarget;
        ...
        var _aSortableOptions = {
                items: _bIsTabbed
                    ? '> li:not( .disabled )'
                    : '.on-screen-keyboard-section:not( .disabled, .on-screen-keyboard-collapsible-toggle-all-button-container )',
                handle: _bCollapsible
                    ? '.on-screen-keyboard-section-caption'
                    : false,
                ...
                stop: function(e,ui) {
                    ...
                    // Callback the registered callback functions.
                    $( this ).trigger(
                        'on-screen-keyboard_stopped_sorting_sections',
                ...
                // cancel: '.on-screen-keyboard-section-description, .on-screen-keyboard-section-title'
        ...
        if ( ! _bIsTabbed ) {
            ...
            _oSortable.on( 'sortstop', function() {
                ...
                $( this ).find( 'caption > .on-screen-keyboard-section-title:not(.on-screen-keyboard-collapsible-sections-title,.on-screen-keyboard-collapsible-section-title)' ).first().show();
                $( this ).find( 'caption > .on-screen-keyboard-section-title:not(.on-screen-keyboard-collapsible-sections-title,.on-screen-keyboard-collapsible-section-title)' ).not( ':first' ).hide();
  ...
  function debugLog( ...message ) {
    if ( ! parseInt( OnScreenKeyboardAdminPageFrameworkScriptFormMain.debugMode ) ) {
      return;
    }
...
(function($){
  ...
  $.widget( 'on-screen-keyboard.pointerTooltip', $.wp.pointer, {
   /**
 ...
 *
 * It provides the $[ 'on-screen-keyboard-tooltip' ]( options ) jQuery plugin method.
 * The name uses hyphens for the user's text domain to be replaced. So the compiled framework script will be entirely ported with without the keyword of `on-screen-keyboard`.
 *
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'on-screen-keyboard-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `on-screen-keyboard-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="on-screen-keyboard-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'on-screen-keyboard-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * ```
 *
 * If it the script is loaded, elements with the .on-screen-keyboard-from-tooltip class will be automatically parsed.
 * So not to call the `[ 'on-screen-keyboard-form-tooltip' ]()` method, just create an element with the selector
 * and it should automatically have a tooltip.
 *
 *
 * When the framework file is compiled, replace the keyword `on-screen-keyboard` with your text domain.
 */
(function($){

  // Initialize
  $( document ).ready( function() {
    $( '.on-screen-keyboard-form-tooltip' ).each( function() {
      $( this )[ 'on-screen-keyboard-form-tooltip' ]();
    } );
  } );

  $.fn[ 'on-screen-keyboard-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'on-screen-keyboard-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.on-screen-keyboard-form-tooltip-content' ).clone();
    ...
    var _options    = $.extend( true, {}, options, {
      ...
      position: {
        ...
        within: _offscreen ? _body : $( self ).closest( '.on-screen-keyboard-field, .on-screen-keyboard-fieldrow, .on-screen-keyboard-section' ),
      },
      buttons: function() {},
      close: function() {},
    }, options );
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.on-screen-keyboard-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.on-screen-keyboard-form-tooltip-balloon' ).on( 'mouseenter', function(){
    ...
    $( document ).ready( function() {
        ...
        $( document ).ajaxComplete( function( event, XMLHttpRequest, ajaxOptions ) {
            ...
            if( _aRequest.action && ( 'save-widget' === _aRequest.action ) ) {
                ...
                if ( $( _oWidget ).find( '.on-screen-keyboard-sectionset' ).length <= 0 ) {
                    return;
                }
                ...
                $( document ).trigger(
                    'on-screen-keyboard_saved_widget',
                    _oWidget
                );
