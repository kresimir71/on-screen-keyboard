...
class OnScreenKeyboardAdminPageFramework_View__PageMetaboxEnabler extends OnScreenKeyboardAdminPageFramework_FrameworkUtility {
    public $oFactory;
    ...
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, array( 'aOnScreenKeyboardAdminPageFramework', 'aMetaBoxForPagesClasses' ));
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, array( 'aOnScreenKeyboardAdminPageFramework', 'bAddedMetaBoxScript' ), false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS[ 'aOnScreenKeyboardAdminPageFramework' ][ 'bAddedMetaBoxScript' ] = true;
        $_sScript = <<<JAVASCRIPTS
jQuery(document).ready(function(){postboxes.add_postbox_toggles(pagenow)})
JAVASCRIPTS;
        echo '<script class="on-screen-keyboard-insert-metabox-script">' . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . '</script>';
    }
}
