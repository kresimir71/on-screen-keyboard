...
class OnScreenKeyboardAdminPageFramework_View__PageRenderer__InPageTabs extends OnScreenKeyboardAdminPageFramework_FrameworkUtility {
    public $oFactory;
    public $sPageSlug;
    public $sTag = 'h3';
    ...
    private function _getOutput($aInPageTabs, $sCurrentPageSlug, $sTag)
    {
        ...
        if (! $_aPage[ 'show_in_page_tabs' ]) {
            return $this->getElement($aInPageTabs, array( $_sCurrentTabSlug, 'title' )) ? "<{$_sTag} class='on-screen-keyboard-in-page-tab-title'>" . $aInPageTabs[ $_sCurrentTabSlug ][ 'title' ] . "</{$_sTag}>" : "";
        }
    ...
    {
        $_oTabBar = new OnScreenKeyboardAdminPageFramework_TabNavigationBar($aTabs, $sActiveTab, $sTag, array( 'class' => 'in-page-tab', ), array( 'format' => array( $this, '_replyToFormatNavigationTabItem_InPageTab' ), 'arguments' => array( 'page_slug' => $sCurrentPageSlug, ), ));
        $_sTabBar = $_oTabBar->get();
        return $_sTabBar ? "<div class='on-screen-keyboard-in-page-tab'>" . $_sTabBar . "</div>" : '';
    }
    public function _replyToFormatNavigationTabItem_InPageTab(array $aTab, array $aStructure, array $aTabs, array $aArguments=array())
    {
        $_oFormatter = new OnScreenKeyboardAdminPageFramework_Format_NavigationTab_InPageTab($aTab, $aStructure, $aTabs, $aArguments, $this->oFactory);
